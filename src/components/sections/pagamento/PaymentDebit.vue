...
.payment-debit__selected-method {
  display: flex;
  align-items: center;
  gap: var(--spacing-md);
...
.payment-debit__radio-indicator {
  display: block;
  width: 23px;
  height: 22px;
...
.payment-debit__field {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.payment-debit__field--half {
  flex: 0 0 auto;
}
...
.payment-debit__input::placeholder {
  color: rgba(0, 0, 0, 0.55);
}

.payment-debit__input:focus {
  outline: none;
  border-color: var(--color-primary);
  box-shadow: 0 0 0 2px rgba(41, 16, 0, 0.1);
}

.payment-debit__row .payment-debit__input {
  width: 179px;
  max-width: 179px;
}

.payment-debit__checkbox {
  display: flex;
  align-items: center;
  gap: var(--spacing-sm);
  cursor: pointer;
...
.payment-debit__checkbox-custom {
  display: block;
  width: 23px;
  height: 22px;
...
.payment-debit__checkbox-input:checked + .payment-debit__checkbox-custom {
  background-color: var(--color-primary);
  border-color: var(--color-primary);
}

.payment-debit__checkbox-input:checked + .payment-debit__checkbox-custom::after {
  content: '';
  position: absolute;
  top: 3px;
  left: 7px;
...
.payment-debit__checkbox-input:focus-visible + .payment-debit__checkbox-custom {
  outline: 2px solid var(--color-primary);
  outline-offset: 2px;
}

.payment-debit__checkbox-label {
  font-family: var(--font-primary);
  font-weight: 300;
  font-size: var(--text-base);
  line-height: 1.25em;
...
.payment-debit__btn--outline {
  background-color: var(--color-white);
  color: var(--color-primary);
  border: 1px solid var(--color-primary);
}

.payment-debit__btn--outline:hover {
  background-color: rgba(41, 16, 0, 0.04);
}

.payment-debit__btn--primary {
  background-color: var(--color-primary);
...
.payment-debit__btn--primary:hover {
  opacity: 0.9;
}
